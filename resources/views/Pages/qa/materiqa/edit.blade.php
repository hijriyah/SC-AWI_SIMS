...
$(document).ready(function () {
    ...
    FilePond.setOptions({
        ...
        server: {
            process: {
                url: "{{ route('qa_materiqa_update', ['uuid' => $dataEdit->uuid]) }}",
                method: 'POST',
                headers: {
    ...
    $.ajax({
        ...
        success: (response) => {
            ...
            $('#materiQaForm').on('submit', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        setTimeout(() => {
                            window.location.href = "{{ route('qa_materiqa_page') }}";
                        }, 2000);
                    },
           ...
           <div class="card-body">
                <div class="position-custom">
                    <div class="d-flex justify-content-between">
                        ...
                        <div></div>
                        <!--<a href="{{ route('qa_materiqa_page') }}" class="btn btn-sm btn-danger" style="height: 29px;"><i class="mdi mdi-keyboard-backspace"></i> Kembali</a>-->
                    </div>
                </div>

               {{-- <p class="card-title-desc">Examples of twitter bootstrap datepicker.</p> --}}

               <form id="materiQaForm" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        ...
                        <textarea class="form-control" name="kompetensi" style="height: 100px">{{ $dataEdit->kompetensi }}</textarea>
                        @error('kompetensi')
                            <span class="text-sm text-danger">{{ $message }}</span>
                        @enderror
                        </div>
