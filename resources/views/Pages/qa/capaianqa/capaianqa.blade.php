        ...
        $('.delete-button').on('click', function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "/administrator/dashboard/qa/capaianqa/delete/" + uuid,
                        type: 'POST',
                        data: {
                ...
                <div class="card-body">
                    <div class="display-position" style="margin-bottom: 20px;">
                        <div class="d-flex justify-content-between">
                            <div style="margin-top: 0px;">
                                <h4 class="card-title" style="width: 200px;"><b>Daftar Capaian QA</b></h4>
                            </div>
                            ...
                            <div style="margin-top: 0px; width: 100px;">
                                <a class="btn btn-sm btn-success" href="{{ route('qa_capaianqa_add') }}"><i class="mdi mdi-plus"></i> Tambah</a>
                            </div>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-editable table-nowrap align-middle table-edits">
                            ...
                            <tbody id="table-body">
                                @foreach ($dataCapaianQa as $item)
                                    <tr>
                                       <td>{{ $loop->iteration }}</td>
                                       <td>{{ $item->kelas->nama_kelas}} </td>
                                       <td>{{ $item->siswa->nama_lengkap}} </td>
                                       <td>{{ $item->materiqa->materi}} </td>
                                       <td>{{ $item->hasil_perkembangan}} </td>
                                       <td>{{ $item->status}} </td>
      
                                        <td style="width: 100px">
                                            <a class="btn btn-outline-info btn-sm edit" title="Edit" href=" {{ route('qa_capaianqa_edit', ['uuid' => $item->uuid]) }} ">
