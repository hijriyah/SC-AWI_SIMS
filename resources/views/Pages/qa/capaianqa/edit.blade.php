           ...
           <div class="card-body">
                <div class="position-custom">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mb-3"><b>Edit Capaian QA</b></h4>
                        <div></div>
               ...
               <form action="{{ route('qa_capaianqa_update', ['uuid' => $dataEdit->uuid]) }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="row">
                      ...
                      <div class="col-lg-6">
                           <div class="mt-4 mt-lg-0 mb-3">
                              ...
                              <select class="form-select" placeholder="Pilih" name="id_materi_qa">
                                  ...
                                  <option value="{{ $materiqa->id }}" {{ old('id_materi_qa', $materiqa->id ) == $dataEdit->id_materi_qa ? 'selected' : ''}}>{{ $materiqa->materi }}</option>
                                  @endforeach
                              </select>
                         </div>
                        </div>
                      ...
                      <div class="col-md-12">
                            ...
                            <input type='textarea' class="form-control" name="kompetensi" value="{{ $dataEdit->kompetensi }}" />
                      </div>
                      ...
                      <div class="col-lg-6">
                        ...
                        <div class="col-lg-12 mb-4">
                            ...
                            <select class="form-select" name="status">
                                <option selected disabled>Pilih</option>
                                <option value="tuntas" {{ old('status', $dataEdit->status) == "tuntas" ? 'selected' : ''}}>Tuntas</option>
                                <option value="tuntas_dengan_remidi" {{ old('status', $dataEdit->status) == "belum_dengan_remidi" ? 'selected' : ''}}>Tuntas dengan Remidi</option>
                                <option value="belum_tuntas" {{ old('status', $dataEdit->status) == "belum_tuntas" ? 'selected' : ''}}>Belum Tuntas</option>
                            </select>
                        </div>
