...
<script>
    ...
    function SearchData()
    {
        ...
        $.ajax({
            url: "{{ route('arsipguru_kategoriarsip_page') }}",
            type: "GET",
            data: { search: search},
        ...
        $('.delete-button').on('click', function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "/administrator/dashboard/arsipguru/kategoriarsip/delete/" + uuid,
                        type: 'POST',
                        data: {
                ...
                <div class="card-body">
                    <div class="display-position" style="margin-bottom: 20px;">
                        <div class="d-flex justify-content-between">
                            <div style="margin-top: 0px;">
                                <h4 class="card-title" style="width: 200px;"><b>Daftar Kategori Arsip</b></h4>
                            </div>
                            ...
                            <div style="margin-top: 0px; width: 100px;">
                                <a class="btn btn-sm btn-success" href="{{ route('arsipguru_kategoriarsip_add') }}"><i class="mdi mdi-plus"></i> Tambah</a>
                            </div>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-editable table-nowrap align-middle table-edits">
                            ...
                            <tbody id="table-body">
                                @foreach ($dataKategoriArsip as $item)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $item->kategori_arsip }} </td>
                                        <td>{{ $item->deskripsi }} </td>
                                        <td style="width: 100px">
                                            <a class="btn btn-outline-info btn-sm edit" title="Edit" href=" {{ route('arsipguru_kategoriarsip_edit', ['uuid' => $item->uuid]) }} ">
