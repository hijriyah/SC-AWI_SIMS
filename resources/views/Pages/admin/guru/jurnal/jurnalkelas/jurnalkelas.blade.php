...
<script>
    ...
    function SearchData()
    {
        ...
        $.ajax({
            url: "{{ route('guru_jurnal-jurnalkelas_page') }}",
            type: "GET",
            data: { search: search},
        ...
        $('.delete-button').on('click', function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "/guru/dashboard/jurnal/jurnalkelas/delete/" + uuid,
                        type: 'POST',
                        data: {
                    ...
                    <div class="display-position" style="margin-bottom: 20px;">
                        <div class="d-flex justify-content-between">
                            ...
                            <div style="margin-top: 0px; width: 100px;">
                                <a class="btn btn-sm btn-success" href="{{ route('guru_jurnal-jurnalkelas_add') }}"><i class="mdi mdi-plus"></i> Tambah</a>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <tr>
                                       ...
                                       <td class="{{ $item->class }}" style="padding: 5px; transform: translateY(13px);">{{ $item->status}} </td>
                                        <td style="width: 100px">
                                            <a class="btn btn-outline-danger btn-sm" title="Export To Pdf" href="{{ route('guru_jurnal-jurnalkelas_download', ['id' => $item->id]) }}">
                                                <i class="fas fa-file-pdf"></i>
                                            </a>
                                            <a class="btn btn-outline-info btn-sm edit" title="Edit" href=" {{ route('guru_jurnal-jurnalkelas_edit', ['uuid' => $item->uuid]) }} ">
