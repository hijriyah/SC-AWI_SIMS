...
$(document).ready(function () {
    ...
    FilePond.setOptions({
        ...
        server: {
            process: {
                url: "{{ route('guru_akademik-tugassiswa_update', ['uuid' => $dataEdit->uuid]) }}",
                method: 'POST',
                headers: {
                     ...
                     <div class="col-lg-6 mb-4">
                        ...
                        <select class="form-select" placeholder="Pilih" name="id_mata_pelajaran">
                           ...
                           @foreach($dataMataPelajaran as $matapelajaran)
                              <option value="{{ $matapelajaran->id }}" {{ old('id_mata_pelajaran', $matapelajaran->id) == $dataEdit->id_mata_pelajaran ? 'selected' : ''}}>{{ $matapelajaran->matapelajaran }}</option>
                           @endforeach
                       </select>
