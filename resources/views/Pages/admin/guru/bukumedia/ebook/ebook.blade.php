...
<script>
    ...
    function SearchData()
    {
        ...
        $.ajax({
            url: "{{ route('guru_bukumedia-ebooks_page') }}",
            type: "GET",
            data: { search: search},
        ...
        $('.delete-button').on('click', function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "/guru/dashboard/bukumedia/ebooks/delete/" + uuid,
                        type: 'POST',
                        data: {
                    ...
                    <div class="display-position" style="margin-bottom: 20px;">
                        <div class="d-flex justify-content-between">
                            ...
                            <div style="margin-top: 0px; width: 100px;">
                                <a class="btn btn-sm btn-success" href="{{ route('guru_bukumedia-ebooks_add') }}"><i class="mdi mdi-plus"></i> Tambah</a>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody id="table-body">
                                @foreach ($dataEbooks as $item)
                                    <tr>
                                        ...
                                        <td><img width="50" height="50" src="{{ '/storage/' . $item->coverfoto}}"/>" </td>
                                        <td style="width: 150px">
                                            <a class="btn btn-outline-warning btn-sm edit" title="preview" href="{{ '/storage/' . $item->file }} " target="_blank">
                                                <i class="fas fa-file-alt"></i>
                                            </a>
                                            <a class="btn btn-outline-info btn-sm edit" title="Edit" href=" {{ route('guru_bukumedia-ebooks_edit', ['uuid' => $item->uuid]) }} ">
