...
<script>
    ...
    $(document).ready(function () {
        ...
        FilePond.setOptions({
            server: {
                process: {
                    url: "{{ route('ujian_banksoal_update', ['uuid' => $dataEdit->uuid]) }}",
                    method: 'POST',
                    headers: {
    ...
    $.ajax({
            ...
            $('#BankSoalForm').on('submit', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        setTimeout(() => {
                            window.location.href = "{{ route('ujian_banksoal_page') }}";
                        }, 2000);
                    },
        ...
        $('#JumlahPilihan').on('change', function() {
            ...
            $.ajax({
                url: "{{ route('ujian_banksoal_edit', ['uuid' => $dataEdit->uuid]) }}",
                type: 'GET',
                data: {
                    selected: Jpilihan
                },
           ...
           <div class="card-body">
               ...
               <form enctype="multipart/form-data" id="BankSoalForm">
                    ...
                    <div class="d-flex justify-content-end">
                        ...
                        <a href="{{ route('ujian_banksoal_page') }}" class="btn btn-sm btn-danger" style="height: 29px;"><i class="mdi mdi-keyboard-backspace"></i> Kembali</a>
                    </div>
               </form>
           </div>
       </div>
